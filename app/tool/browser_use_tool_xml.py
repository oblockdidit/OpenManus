...
"""
                    ...
                    # Use LLMIntegration instead of direct LLM
                    try:
                        ...
                        full_response = response.get("full_text", "")
                        
                        logger.info(f"XML Extract content response: {len(full_response)} chars, {len(tool_calls)} tool calls")
                        
                        # Look for extract_content tool calls
                        extracted_content = None
                        for tool_call in tool_calls:
                            if tool_call.name == "extract_content":
                                extracted_content = tool_call.parameters.get("text", "")
                                break
                        
                        # If no tool call was found, parse the text content manually
                        if not extracted_content and full_response:
                            # Try using the parser again on the full response
                            _, tool_calls = parse_assistant_message(full_response)
                            for tool_call in tool_calls:
                                if tool_call.name == "extract_content":
                                    extracted_content = tool_call.parameters.get("text", "")
                                    break
                        
                        # If still no extracted content, use the text content or full response
                        if not extracted_content:
                            extracted_content = text_content or full_response
                        ...
                        return ToolResult(output=f"Extracted from page: {json.dumps(output)}")
                    
                    except Exception as extraction_error:
                        logger.error(f"Error with XML extraction: {extraction_error}")
                        
                        # Fall back to the old method using function calling as a backup
                        try:
                            function_prompt = f"""\
Your task is to extract content from a web page. You will be given a page and a goal, and you should extract all relevant information around this goal from the page. If the goal is vague, summarize the page. Respond in json format.
Extraction goal: {goal}

Page content:
{content[:max_content_length]}
"""
                            function_messages = [{"role": "system", "content": function_prompt}]
                            ...
                            logger.info("Falling back to native function calling")
                            ...
                            # If native function calling fails too, just return what we have
                            return ToolResult(output="No content was extracted from the page.")
                            
                        except Exception as function_error:
                            logger.error(f"Function calling extraction also failed: {function_error}")
                            
                            # Last resort - manual extraction
                            return ToolResult(
                                output=f"Extracted from page: {{'status': 'success', 'goal': '{goal}', 'extracted_content': {{'text': 'Extraction failed due to API issues. Page appears to contain: {content[:200]}...'}} }}"
                            )
