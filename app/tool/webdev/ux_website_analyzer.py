...
class UXWebsiteAnalyzer(BaseTool):
    """
    UX-focused website analyzer that uses navbar exploration to analyze websites.

    This tool uses direct Playwright methods to:
    - Identify and navigate through navbar links
    - Capture screenshots of each page
    - Use vision models to analyze page content with focus on UX/UI issues
    - Generate comprehensive website analysis
    """

    name: str = "website_analyzer"
    description: str = (
        "Performs comprehensive website analysis with focus on UX/UI issues by navigating through navbar links "
    ...
    async def execute(
        ...
        """
        Execute comprehensive website analysis using navbar exploration with focus on UX/UI issues.
        ...
        try:
            ...
            try:
                ...
                # Store UX issues for later analysis
                results["ux_issues"].append(
                    {"page": "homepage", "analysis": homepage_analysis}
                )
                    ...
                    try:
                        ...
                        for link_element in all_links:
                            ...
                            if link.lower() in text_content.lower():
                                ...
                                # Analyze the page with vision model focusing on UX/UI issues
                                page_prompt = (
                                    f"This is the {link} page of a website. Analyze it with a focus on UX/UI issues. "
                                    f"Identify specific problems with: \n"
                                    f"1. Layout and visual hierarchy\n"
                                    f"2. Navigation and user flow\n"
                                    f"3. Responsiveness and mobile-friendliness\n"
                                    f"4. Color scheme and typography\n"
                                    f"5. Call-to-action buttons and forms\n"
                                    f"6. Content organization and readability\n\n"
                                    f"For each issue, explain why it's a problem and how it impacts the user experience. "
                                    f"Be specific and detailed in your analysis."
                                )
                                ...
                                # Store UX issues for later analysis
                                results["ux_issues"].append(
                                    {"page": link, "analysis": page_analysis}
                                )
                ...
                # Calculate scores based on UX/UI analysis
            ...
            # Save detailed UX/UI analysis for each page to files
            if results.get("ux_issues"):
                analysis_dir = "website_analysis_reports"
                os.makedirs(analysis_dir, exist_ok=True)

                for page_analysis in results.get("ux_issues", []):
                    page_name = page_analysis.get("page", "unknown")
                    analysis_text = page_analysis.get("analysis", "")

                    # Create a filename based on domain and page name
                    timestamp = int(time.time())
                    filename = f"{domain}_{page_name.replace(' ', '_')}_{timestamp}.txt"
                    filepath = os.path.join(analysis_dir, filename)

                    # Save the analysis to a file
                    with open(filepath, "w") as f:
                        f.write(f"UX/UI Analysis for {page_name} page of {url}\n")
                        f.write("=" * 80 + "\n\n")
                        f.write(analysis_text)

                    logger.info(
                        f"Saved detailed UX/UI analysis for {page_name} page to {filepath}"
                    )

                    # Add the filepath to the page analysis
                    page_analysis["analysis_file"] = filepath
            ...
            final_results = {
                ...
                "page_analysis_files": [
                    page.get("analysis_file")
                    for page in results.get("ux_issues", [])
                    if page.get("analysis_file")
                ],
            }
    ...
    def _calculate_scores(self, results: Dict[str, Any]) -> Dict[str, int]:
        ...
        try:
            # Get the UX/UI analysis from all pages
            ux_issues = results.get("ux_issues", [])
            ...
            # Analyze the UX/UI issues for each page
            for page_issues in ux_issues:
                analysis = page_issues.get("analysis", "")

                # Define UX/UI issue categories and their keywords
                ux_issue_categories = {
                    "layout": [
                        "layout",
                        "visual hierarchy",
                ...
                issue_counts = {category: 0 for category in ux_issue_categories}

                for category, keywords in ux_issue_categories.items():
