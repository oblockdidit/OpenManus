...
from app.tool.webdev.enhanced_website_analyzer import EnhancedWebsiteAnalyzer
...
class LeadProspectorAgent(Manus):
    ...
    available_tools: ToolCollection = Field(
        default_factory=lambda: ToolCollection(
            EnhancedWebsiteAnalyzer(), Terminate()
        )
    )
    ...
    async def analyze_single_lead(self, company_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Analyzing {company_name} with domain {domain_name} in the {industry} industry")
            
            # Explicitly analyze the website first using the EnhancedWebsiteAnalyzer tool
            website_analyzer = EnhancedWebsiteAnalyzer()
            ...
            try:
                ...
                website_data = {
                    ...
                    "website_status": "Basic Website" if website_results.get("exists", False) else "No Website",
                    "website_url": website_results.get("url", f"https://{domain_name}"),
                    "design_score": website_results.get("scores", {}).get("design", 65),
                    "performance_score": website_results.get("scores", {}).get("performance", 40),
                    "mobile_score": website_results.get("scores", {}).get("mobile", 40),
                    "seo_score": website_results.get("scores", {}).get("seo", 25),
                    "technologies": website_results.get("technologies", ["HTML", "CSS", "JavaScript"]),
                    "improvement_suggestions": "\n".join([f"- {item}" for item in website_results.get("improvement_opportunities", ["Add responsive design", "Improve page load speed", "Add SEO metadata"])]),
                ...
                website_summary += f"- Technologies: {', '.join(website_data['technologies'])}\n"
                ...
                logger.error(f"Error during direct website analysis: {str(website_error)}")
            ...
            try:
                ...
                await self.crm_connector.update_company_rest(
                    ...
                    data={
                        ...
                        "proposedSolution": industry_recommendations.get("proposed_solution", ""),
                ...
                if "invalid input syntax for type uuid" in str(update_error):
                    logger.warning("Invalid UUID format in company_id. To fix this, please use a valid UUID format in the _id field.")
            ...
            if website_data.get("has_website", False):
                ...
                technologies_text = ', '.join(technologies) if technologies else "Basic HTML, CSS, JavaScript"
                ...
                web_analysis += f"Pages Analyzed: {len(website_data.get('pages_analyzed', []))}\n"
                
                # Add list of pages if available
                if website_data.get('pages_analyzed'):
                    web_analysis += "\nAnalyzed pages:\n"
                    for i, page in enumerate(website_data.get('pages_analyzed', [])[:5], 1):
                        page_url = page
                ...
                try:
                    ...
                    logger.error(f"Error creating website analysis note via REST: {str(rest_error)}")
                    # Try simplifying the data if there's a field error
                    if "Unknown argument" in str(rest_error) or "Cannot query field" in str(rest_error):
                        try:
                            ...
                            logger.error(f"Error creating simplified note: {str(simple_error)}")
                    else:
                        try:
                            ...
                            logger.error(f"Error creating website analysis note via GraphQL: {str(graphql_error)}")
                            if "Cannot query field" in str(graphql_error):
                                logger.warning("Note schema error. Check the field names in the GraphQL query.")
        ...
        for msg in self.memory.messages:
            if msg.role == "tool" and "website_analyzer" in msg.content:
                try:
                    ...
                    if 'website_analyzer' in msg.content and '{' in msg.content and '}' in msg.content:
                        ...
                        if scores:
                            ...
                            analysis_data["performance_score"] = scores.get("performance", 50)
                        ...
                        if "improvement_opportunities" in tool_data:
                            analysis_data["improvement_suggestions"] = "\n".join([f"- {item}" for item in tool_data["improvement_opportunities"]])
                    ...
                    try:
                        ...
                        logger.info(f"Tool content for debugging: {msg.content[:200]}...")
                    except Exception:
                        pass
    ...
    def _process_analysis_data(self, analysis_data: Dict[str, Any], domain_name: str) -> Dict[str, Any]:
        """
        ...
        if 'has_website' in analysis_data and analysis_data['has_website'] and not analysis_data['website_url']:
            ...
            analysis_data['improvement_suggestions'] = """- Add responsive viewport meta tag for proper mobile rendering
- Add meta description for better search engine visibility
...
- Implement HTTPS for security and SEO benefits"""
    ...
    async def _generate_industry_recommendations(
        ...
        Pages Analyzed: {len(website_data.get('pages_analyzed', []))}
        Screenshots Captured: {len(website_data.get('screenshots', []))}
        
        Top Issues: {', '.join([issue.get('issue', '') for issue in website_data.get('details', {}).get('seo_issues', {}).get('top_issues', [])[:3]]) if website_data.get('details', {}).get('seo_issues', {}).get('top_issues') else 'None identified'}
        ...
        try:
            # Direct call to LLM - streamlined approach
            response = await self.llm.ask(
                messages=[{"role": "user", "content": prompt}],
                temperature=0.7,
                stream=False  # No streaming to avoid potential issues
            )
            ...
            for line in lines:
                ...
                if current_section == "questions" and line.strip() and line.strip()[0].isdigit():
                    questions.append(line.strip())
                elif current_section == "talking_points" and line.strip() and line.strip()[0].isdigit():
                    talking_points.append(line.strip())
                elif current_section == "objections" and line.strip() and line.strip()[0].isdigit():
            ...
            # Return fallback recommendations if direct LLM call fails
            return {
                "questions_to_ask": [
